...
- `GET /api/products?business={shop-slug}`
- `GET /api/clinics`
...
- `GET /api/admin/merchants?includeInactive=true`
- `DELETE /api/admin/merchants/{id}`

Merchant accounts use their own login cookie and can only update their linked shop, including merchant-created products, deals, and offers:

- `GET /api/merchant/me`
- `PATCH /api/merchant/business`
- `POST /api/merchant/products`
- `PATCH /api/merchant/products/{slug}`
- `DELETE /api/merchant/products/{slug}`
- `POST /api/merchant/deals`
- `PATCH /api/merchant/deals/{slug}`
- `DELETE /api/merchant/deals/{slug}`

Merchant product JSON includes catalog/display fields for Flutter and web:

- `unit`, `unitOptions`, `sizeOptions`
- `galleryUrls`
- `stockStatus` and `isSoldOut`
- `deliveryEnabled`, `deliveryAreas`, `deliveryFee`, `deliveryEta`, `deliveryContact`
- `orderInstructions`
