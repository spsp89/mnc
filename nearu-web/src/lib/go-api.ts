...
export async function getGoAppSettings() {
  const data = await readApi<{ settings: GoAppSettings }>("/api/app-settings");
  return data.settings;
}

export async function updateGoAppSettings(input: GoAppSettings) {
  const data = await writeApi<{ settings: GoAppSettings }>("/api/admin/app-settings", "PATCH", input);
  return data.settings;
}
