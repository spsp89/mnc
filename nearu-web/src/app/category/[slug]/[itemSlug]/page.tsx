...
import { categoryItemGroups, getCategoryItem, getCategoryItemGroup, getCategoryItemProducts, getCategoryItems, getSubcategoryProducts } from "@/lib/category-items";
...
export async function generateMetadata({ params }: CategoryItemPageProps): Promise<Metadata> {
  ...
  const categoryName = category?.name ?? categoryGroup?.categoryName ?? titleFromSlug(slug);
...
export default async function CategoryItemPage({ params }: CategoryItemPageProps) {
  ...
  const categoryName = category?.name ?? categoryGroup?.categoryName ?? titleFromSlug(slug);
  const itemProducts = getCategoryItemProducts(categoryName, item);
    ...
    <div className="min-h-screen bg-[#fffdf7] text-[#0b2f74]">
      ...
      <main className="mx-auto grid max-w-[1800px] gap-6 px-4 py-8 sm:px-5 md:px-10 lg:grid-cols-[1fr_380px]">
        <section>
            ...
            {item.highlights.map((highlight) => (
              <div key={highlight} className="rounded-[16px] border border-[#dfe6f2] bg-white p-5 shadow-[0_12px_28px_rgba(11,47,116,0.06)]">
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  {getSubcategoryProducts(categoryName, highlight, item.image).slice(0, 3).map((product) => (
                    <span key={product.name} className="rounded-full bg-[#edf3ff] px-3 py-1 text-[11px] font-black text-[#0b2f74]">
                      {product.name}
                    </span>
                  ))}
                </div>
          ...
          <h2 className="mt-8 text-[26px] font-black leading-tight sm:text-[34px]">Products and subcategory deals</h2>
          <div className="mt-5 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            {itemProducts.map((product) => (
              <div key={product.name} className="overflow-hidden rounded-[18px] border border-[#dfe6f2] bg-white shadow-[0_14px_32px_rgba(11,47,116,0.07)]">
                <div className="relative h-[160px] bg-[#f6f8fc]">
                  <div className="absolute inset-0 bg-contain bg-center bg-no-repeat" style={{ backgroundImage: `url(${product.image})` }} />
                  <span className="absolute left-3 top-3 rounded-full bg-[#f5b625] px-3 py-1 text-[11px] font-black text-[#08285f]">{product.badge}</span>
                </div>
                <div className="p-4">
                  <h3 className="line-clamp-2 min-h-[44px] text-[18px] font-black leading-tight text-[#0b2f74]">{product.name}</h3>
                  <p className="mt-2 text-[12px] font-bold text-[#596a82]">{product.code} · Nearby local shops</p>
                </div>
              </div>
            ))}
          </div>
