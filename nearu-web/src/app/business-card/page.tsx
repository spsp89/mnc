...
import { BusinessCardShareActions } from "@/components/nearu/business-card-share-actions";
import { categoryItemGroups } from "@/lib/category-items";
import { absoluteUrl, defaultOpenGraph, pageTitle } from "@/lib/seo";

const sampleBusinessSlug = "spice-garden";
const cardPath = `/business-card/${sampleBusinessSlug}`;
const cardUrl = absoluteUrl(cardPath);
const mapHref = "https://www.google.com/maps/search/?api=1&query=Spice%20Garden%20Kozhikode%20Kerala";
...
const vcardHref = `data:text/vcard;charset=utf-8,${encodeURIComponent([
  ...
  "EMAIL:hello@chamsglobal.com",
  "URL:https://chamsglobal.com",
  "ADR;TYPE=WORK:;;Kozhikode;Kerala;;;India",
  "END:VCARD",
].join("\n"))}`;
...
const contactActions = [
  ...
  { label: "Directions", icon: MapPin, href: mapHref },
  { label: "Share", icon: Share2, href: cardPath },
];
                ...
                <div className="rounded-[14px] bg-white p-4 text-[#10233f]">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[15px] font-black">{cardUrl.replace(/^https?:\/\//, "")}</p>
                    </div>
                  </div>
                  <BusinessCardShareActions
                    cardUrl={cardUrl}
                    cardTitle="Chams Global | BNC Nearu"
                    vcardHref={vcardHref}
                  />
                </div>
